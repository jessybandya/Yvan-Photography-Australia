...
function Post({ fullName, quoteID, message, email, timestamp, number, phone, address, amount, selectedDateTime }) {
  const [loading, setLoading] = React.useState(false);
  const [open, setOpen] = React.useState(false);


   const onDeleteBooking = async () => {
    const confirmDelete = window.confirm(`Are you sure you want to delete booking: "${fullName}'s contact"?`);
    ...
    try {
      ...
      await db.collection("contact-me").doc(quoteID).delete();
  
      toast.success(`"${fullName}'s contact" deleted successfully!`, {
  ...
  return (
    ...
    <DialogHeader color='orange'>Message</DialogHeader>
    <DialogBody>
      {paragraphs}
    </DialogBody>
    <DialogFooter>
